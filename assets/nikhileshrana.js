console.log("Developed By Nikhilesh Rana for more info click on -  https://www.nikhileshrana.tech  ");
  ...
  document.addEventListener('DOMContentLoaded', function() {
    const checkbox = document.getElementById('additional-feature');
    const priceElement = document.querySelector('.price--large'); // Adjust selector as necessary
    const initialPrice = parseFloat(priceElement.textContent.replace(/[^\d.-]/g, "")); // Convert price to float

    checkbox.addEventListener('change', function() {
      const additionalCost = checkbox.checked ? 200 : 0;
      const newPrice = initialPrice + additionalCost;
      priceElement.textContent = newPrice.toFixed(2) + 'rs'; // Adjust currency format as necessary
    });
  });
